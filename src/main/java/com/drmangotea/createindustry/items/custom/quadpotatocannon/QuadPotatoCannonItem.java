package com.drmangotea.createindustry.items.custom.quadpotatocannon;
...
public class QuadPotatoCannonItem extends ProjectileWeaponItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
        ...
        return findAmmoInInventory(world, player, stack).map(itemStack -> {
                    ...
                    if (world.isClientSide) {
                        CreateIndustry.QUAD_POTATO_CANNON_RENDER_HANDLER.dontAnimateItem(hand);
                    ...
                    for (int i = 0; i < projectileType.getSplit()+3; i++) {
                        int offset = (int)Math.floor(Math.random()*(1-(-1)+1)+(-1));
                        int offset2 = (int)Math.floor(Math.random()*(1-(-1)+1)+(-1));
                        int offset3 = (int)Math.floor(Math.random()*(1-(-1)+1)+(-1));
                        ...
                        projectile.setPos(barrelPos.x+offset-0.3, barrelPos.y+offset2-0.3, barrelPos.z+offset3-0.3);
